...
export function PlayGame() {
  ...
  useEffect(() => {
    ...
    async function loadSession() {
      ...
      try {
        ...
        if (!latestSession) {
          setErrorMessage("当前还没有可用的对战会话，请先返回入口页启动。");
          return;
        }
  ...
  useEffect(() => {
    ...
    const timer = window.setInterval(async () => {
      try {
        ...
        // keep gameplay uninterrupted when polling fails transiently
      }
    }, 2000);
  ...
  const handleToggleFullscreen = async () => {
    try {
      ...
      // Ignore fullscreen API failures caused by browser permission/state.
    }
  };
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-white">
      <main className="mx-auto h-screen w-full p-2">
        <div
          ref={containerRef}
          className="relative h-[calc(100vh-1rem)] overflow-hidden rounded-2xl border border-slate-800 bg-black shadow-2xl"
        >
          ...
          {session && canReviewNow ? (
            <div className="absolute bottom-3 right-3 z-10 rounded-xl border border-slate-700 bg-slate-900/90 p-3 shadow-xl backdrop-blur">
              <div className="mb-2 text-sm text-slate-200">当前盘已结束</div>
