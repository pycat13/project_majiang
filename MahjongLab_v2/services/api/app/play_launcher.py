...
class MahjongAiLauncher:
    ...
    def _validate_environment(self) -> None:
        ...
        if not self.settings.mahjong_ai_python.exists():
            missing.append(str(self.settings.mahjong_ai_python))
        if not self.settings.mahjong_ai_websockify.exists():
            missing.append(str(self.settings.mahjong_ai_websockify))
        if missing:
            joined = "; ".join(missing)
